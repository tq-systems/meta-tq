...
do_install:append() {
    case ${MACHINE} in
        tqma8qm*-mba8x) install -m 0644 ${UNPACKDIR}/mba8x.conf ${D}${sysconfdir}/sensors.d
                         ;;
        tqma8mp*-mba8mpxl) install -m 0644 ${UNPACKDIR}/mba8mpxl.conf ${D}${sysconfdir}/sensors.d
                         ;;
    esac
}
